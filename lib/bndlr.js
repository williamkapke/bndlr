...
var fs = require('fs'),
	...
	middleware,
	configs = {},
	absolute = process.platform == "win32"? /^[a-z]:\\/i : /^\//;
	;

//keep backwards compat for awhile  :(
if(!fs.existsSync)
...
exports = module.exports = {
				...
				bundlenames.forEach(function(bundlename){
					...
					for(var i=0;i<files.length;i++){
						...
						if(!(path instanceof Bundle)){
							...
							path = exports.getFilePath({configdir:configdir, type:type, filename:path});
	...
	getFilePath: function(info){
		//check if it is already an absolute path: starts with '\' for unix OR starts like 'c:\' for windows
		if(absolute.test(info.filename))
			return info.filename;

		//info came from a config
		if(info.configdir)
			return path.join(info.configdir, info.type, info.filename+(info.ext||''));

		//info came from a url
		return path.join(exports.webroot, exports.staticDir, info.type, info.filename+(info.ext||''));
	},
...
Object.defineProperties(exports, {
	...
	staticUrl: {
		writable: true,
		value: function(type, filename, min){
			if(filename[0] != '/')
		...
		value: {
			...
			static: function(req, res){
				...
				return function(type, filename){
					return exports.staticUrl(type, filename, min);
				};
			}
		}
	},
