...
module.exports = {
		...
		if(!config){
			...
			types.forEach(function(type){
				...
				var typenode = raw[type],
					...
					modifier = module.exports.getFilePath;
	...
	findConfigs: function(dirs, callback){
		...
		require('child_process').execFile('find', dirs.concat(['-name', module.exports.configName, '-print' ]), function(err, stdout, stderr) {
			var files = stdout.substring(0, stdout.length-1).split('\n');
			callback(files);
		});
	},

	//determines the absolute path to a file on disk based on config information
	getFilePath: function(configdir, type, filename){
